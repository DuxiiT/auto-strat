...
local Starlight = loadstring(game:HttpGet("https://gist.githubusercontent.com/DeadSignalFound/c3b532470e7d9d84d4db4a12ffb87375/raw/3c95f36bbfec8c55794e222edd0492eda1ed5223/Source.lua"))()
...
local function parse_skin_payload(value)
    if type(value) == "string" then
        local chunk = value:match("%b{}") or value:match("%b[]")
        if chunk then
            local ok, parsed = pcall(function()
...
local function pick_skin_from_entry(entry)
    ...
    local direct = entry.Equipped or entry.Selected or entry.Skin or entry.Name or entry.skin or entry.name or entry.selected or entry.current or entry.Current or entry.EquippedSkin
    if type(direct) == "string" and direct ~= "" then
        return direct
    end
    for skin_name, skin_data in pairs(entry) do
        if type(skin_data) == "table" then
            local flag = skin_data.Equipped or skin_data.Selected or skin_data.selected or skin_data.equipped
            if flag then
                return tostring(skin_name)
            end
            local nm = skin_data.Name or skin_data.name
            if type(nm) == "string" and nm ~= "" and (skin_data.Equipped ~= nil or skin_data.Selected ~= nil) then
                return nm
            end
        elseif skin_data == true then
            return tostring(skin_name)
        end
    end
    return nil
...
local function get_skin_from_data(data, tower_name)
    if type(data) ~= "table" then
        return nil
    end
    local direct = data[tower_name]
    local picked = pick_skin_from_entry(direct)
    if picked then
        return picked
    end
    local nested = data.EquippedSkins or data.EquippedSkin or data.Skins or data.TowerSkins or data.Equipped or data.Selected
    if nested then
        return get_skin_from_data(nested, tower_name)
    end
    return nil
end

local function get_skin_from_inventory(inventory_data, tower_name)
    if type(inventory_data) ~= "table" then
        return nil
    end
    local tower_data = inventory_data[tower_name]
    if type(tower_data) ~= "table" then
        return nil
    end
    local direct = tower_data.EquippedSkin or tower_data.Skin or tower_data.SkinName or tower_data.skin or tower_data.skinName
    if type(direct) == "string" and direct ~= "" then
        return direct
    end
    local skins = tower_data.Skins or tower_data.skins or tower_data.SkinCollection
    local picked = pick_skin_from_entry(skins)
    if picked then
        return picked
    end
    if type(skins) == "table" then
        for skin_name, skin_data in pairs(skins) do
            if type(skin_name) == "string" and skin_name ~= "" then
                return skin_name
            end
            if type(skin_data) == "string" and skin_data ~= "" then
                return skin_data
            end
        end
    end
...
local function get_equipped_skin(tower_name, inventory_data)
    local inv_pick = get_skin_from_inventory(inventory_data, tower_name)
    if inv_pick then
        return inv_pick
    end
    local rep = getLocalReplicator()
    if rep then
        local rep_keys = {"EquippedSkins", "Skins", "TowerSkins", "EquippedSkin"}
        for _, key in ipairs(rep_keys) do
            ...
            value = parse_skin_payload(value)
            local picked = get_skin_from_data(value, tower_name)
            if picked then
                return picked
            end
        end
    end
    local search_keys = {"Inventory.EquippedSkins", "Inventory.TowerSkins", "Inventory.Skins", "EquippedSkins", "TowerSkins", "Skins"}
...
local function performEquip(towerName, skinName)
    ...
    local skin_name = skinName
    if not skin_name or skin_name == "" then
        skin_name = "Default"
    end
    pcall(function()
        remote:InvokeServer("Inventory", "Equip", "Skin", towerName, skin_name)
    end)
    remoteEvent:FireServer("Streaming", "SelectTower", towerName, skin_name)
    return true, nil
end
...
equipTowerByName = function(towerName)
    ...
    local equipped_skin = get_equipped_skin(towerName, inventoryData)
    local equipSuccess, equipError = performEquip(towerName, equipped_skin)
    if equipSuccess then
        updateEquippedListAfterEquip(towerName, nil)
