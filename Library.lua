...
local Strategies = Window:Tab({Title = "Strategies", Icon = "clipboard-list"}) do
    ...
    local success, AutoTrialsModule = pcall(function()
        return loadstring(game:HttpGet("https://raw.githubusercontent.com/AmonguszzZ/ModdedAether/refs/heads/main/NewFunc/AutoTrials.lua"))()
    end)

    if not success or type(AutoTrialsModule) ~= "table" then
        AutoTrialsModule = {
            GetInfo = function() return "Unknown", "None", false, {} end,
            Run = function() end
        }
    end

    local trialName, trialTowers, isPassed, missing = AutoTrialsModule.GetInfo()

    local Trial = Strategies:Label({
        Title = "Current Trial: " .. tostring(trialName),
        Desc = "Required Towers/Items: " .. tostring(trialTowers)
    })

    local missingText = (type(missing) == "table" and #missing > 0) and table.concat(missing, ", ") or "None"

    local ReqMet = Strategies:Label({
        Title = isPassed and "Met!" or "Missing Towers",
        Desc = isPassed and "All requirements satisfied." or missingText
    })
    ...
    Strategies:Toggle({
        ...
        Callback = function(v)
            ...
            task.spawn(function()
                local successLoad, module = pcall(function()
                    return loadstring(game:HttpGet("https://raw.githubusercontent.com/AmonguszzZ/ModdedAether/refs/heads/main/NewFunc/AutoTrials.lua"))()
                end)

                if not successLoad or type(module) ~= "table" then return end

                local name, towers, passed, miss = module.GetInfo()

                Trial:SetTitle("Current Trial: " .. tostring(name))
                Trial:SetDesc("Required Towers/Items: " .. tostring(towers))

                if passed then
                    ReqMet:SetTitle("Met!")
                    ReqMet:SetDesc("All requirements satisfied.")
                else
                    local missText = (type(miss) == "table" and #miss > 0) and table.concat(miss, ", ") or tostring(miss)
                    ReqMet:SetTitle("Missing Towers")
                    ReqMet:SetDesc(missText)
                    return
                end
